...
var dispatch = (__webpack_require__(8258)/* .dispatch */ .W);
...
/* harmony export */ });
/* harmony import */ var _dispatch_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(2626);
...
var src_dispatch = __webpack_require__(2626);
...
function continuous_transformer() {
  ...
  scale.domain = function(_) {
    return arguments.length ? (domain = map.call(_, number), clamp === continuous_identity || (clamp = clamper(domain)), rescale()) : domain.slice();
  };
...
var d3_cloud = __webpack_require__(6929);
var d3_cloud_default = /*#__PURE__*/__webpack_require__.n(d3_cloud);
;// CONCATENATED MODULE: ./_assets/javascripts/tagscloud.js



class TagsCloud {
  constructor() {
    const font = 'Helvetica, Tahoma, Arial, STXihei, "华文细黑", "Microsoft YaHei", "微软雅黑", SimSun, "宋体", Heiti, "黑体", sans-serif';
    const tagscloud = src_select('#tagscloud');
    const progress = src_select('#tagscloud .progress');
    const fill = ordinal_ordinal(category10); // 插入 svg
    ...
    function draw(data) {
      ...
      text.enter().append('text').text(d => d.text).attr('text-anchor', 'middle').attr('transform', d => `translate(${[d.x, d.y]})rotate(${d.rotate})`).style('font-size', d => `${d.size}px`).on('click', d => window.scrollTo(`${d.text}-ref`)).style('opacity', 1e-6).style('font-family', d => d.font).style('fill', d => fill(d.text.toLowerCase())).transition().duration(1000).style('opacity', 1);
      text.transition().duration(1000).attr('transform', d => `translate(${[d.x, d.y]})rotate(${d.rotate})`).style('font-size', d => `${d.size}px`);
      text.exit().transition().duration(1000).style('opacity', 1e-6).remove(); // 移除 progress

      progress.style('display', 'none');
    }

    this.layout = d3_cloud_default()().timeInterval(10).font(font).fontSize(d => d.size).rotate(() => ~~(Math.random() * 2) * 90) // 0/90
  ...
  generate(tags) {
    this.progress.style('display', 'block'); // 每次生成重新获取 tagscloud 的宽度

    const rect = this.tagscloud.node().getBoundingClientRect();
    const w = rect.width;
    const h = w / 1.6;
    this.svg.attr('width', w).attr('height', h);
    this.vis.attr('width', w).attr('height', h).attr('transform', `translate(${[w >> 1, h >> 1]})`);
    this.layout.stop().size([w, h]).words(tags.map(d => ({
      text: d.name,
